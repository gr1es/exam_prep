...
static char	*ft_strjoin_free(char *to_free, char *to_keep)
{
	...
	else
	{
		str = malloc((ft_strlen(to_free) + ft_strlen(to_keep) + 1)
				* sizeof(char));
		if (!str)
			return (NULL); // Note: In this specific implementation,
				to_free is NOT freed if malloc fails here.
							// This is handled by freer in GNL.
		i = 0;
		while (to_free[i])
		{
			str[i] = to_free[i];
...
static void	freer(char *stash, char *buffer)
{
	if (stash)
		free(stash);
	if (buffer)
